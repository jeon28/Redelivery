...
### Stage 3 (Finalize) — 미확인

- 버튼 클릭 시 액션: `POST /redelivery/save/<redeliveryNumber>` (+ CSRF)
- `Pending Create` → 최종 확정 상태로 전환 추정 (정확한 후속 상태는 미확인)
- 실행 시 결과 화면, 이메일 발송 여부, 상태 변화 등 추가 캡처 필요
