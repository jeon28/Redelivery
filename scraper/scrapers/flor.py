...
# 컨테이너 번호 형식 검증.
# ISO 6346 기반: 4 영문 + 6~7 숫자. FLOR (Florens) 는 DFSU / FSCU / FCIU / FBLU 발급.
# 그 외 prefix 거나 format 불일치면 위저드 호출 전 즉시 '넘버오류' 처리.
_CONTAINER_RE = re.compile(r"^[A-Z]{4}\d{6,7}$")
_FLOR_PREFIXES = ("DFSU", "FSCU", "FCIU", "FBLU")
...
class FlorScraper(BaseScraper):
    ...
    async def query(
        ...
        # + 컨테이너 형식/Prefix 사전 검증: FLOR 은 DFSU/FSCU/FCIU/FBLU 발급.
        ...
        for c in number_errors:
            results[c] = {
                ...
                "over_caps": None,
                "close_date": None,
                "reason": "타사 컨테이너 또는 번호 형식 오류 (FLOR 는 DFSU/FSCU/FCIU/FBLU 만 지원)",
            }
        if number_errors:
